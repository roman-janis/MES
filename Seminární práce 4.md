...
Za data lze považovat samostatně uložené hodnoty. Tyto hodnoty sami o sobě nemusí být nositelem žádného širšího významu.
Informace pak vznikají právě provázáním samostatných hodnot v určitém kontextu a vytvořením struktur. [Pokorný a Valenta, 2020; Watt a Eng, 2014]
...
****
Systém řízení báze dat, běžně označovaný zkratkou DBMS, je specializovaný program, který umožňuje manipulaci, ukládání, zabezpečení i správu dat, která jsou uložena v databázi.
DBMS také v širším kontextu řeší problém transakčního zpracování, obnovení dat po pádu, přístup uživatelů v jednom čase i řízení ochrany dat. [Pokorný a Valenta (2020, s. 17)]
Spojením DBMS společně s daty dohromady tvoří celek, označovaný jako databáze. [Elmasri a Navathe (2016, s. 5)]
****
...
****
Původně se používal především jednoduchý souborový přístup, který měl včas řadu problémů a omezení v podobě konzistentnosti dat při aktualizaci, závislosti na aplikačním programu a na fyzické struktuře, často spojenou s redundancí dat. [Elmasri a Navathe (2016, s. 6)] Databázový přístup tyto nedostatky odstraňuje a to tím, že integruje data do jednoho logického celku a odděluje definici a data od samotných aplikací.
Oddělením bylo získána vyšší bezpečnost, možnost centrálního řízení integritních omezení a sdílenost dat mezi více uživateli. [Pokorný a Valenta (2020, s. 16)]
****
...
****
V teorii databází je třeba rozlišovat mezi schématem databáze a instanci databáze. 
Schéma databáze představuje popis struktury uložených dat a běžně se pod něj zahrnují také integritní omezení. Tedy určení entit, atributů, vazeb a omezení, která mají data splňovat. 
[Pokorný a Valenta (2020, s. 17)]
Pojmem instance databáze naopak vyjadřuje konkrétní aktuální obsah databáze v určitém čase. 
Toto rozlišení odděluje relativně stabilní strukturální návrh od proměnlivých datových hodnot. [Elmasri a Navathe (2016, s. 29)*:]
****
...
****
S databázovým systémem souvisejí také pojem metadata. 
Metadata jsou definice obsažená ve schématech, tedy informace popisující strukturu databáze, význam atributů, integritní omezení nebo přístupová práva. 
Metadata bývají ukládána v systémovém katalogu, který slouží jako centrální zdroj informací o databázových objektech. Metadata popisují strukturu primární databáze. (Elmasri a Navathe, 2016; Pokorný a Valenta, 2020).
****
...
Tato architektura byla realizována impelemntací jistých abstrakcí. Rozlišuje externí, konceptuální a interní úroveň pohledu na data, kde 
...
Při hodnocení nástrojů proto bude vhodné sledovat další možnosti vytváření diagramů, ale také podporu přechodu mezi jednotlivými úrovněmi návrhu a implementace
...
****
Na konceptuální návrh navazuje v dalšá části logický návrh. 
V této části se konceptuální model převádí do zvoleného datového modelu, v případě této práce jde zejména o model relační. 
Dochází k návrhu relací, atributů, klíčů, cizích klíčů a integritních omezení. [Pokorný a Valenta, 2020]
Výsledkem je pak logické schéma databáze, které lze implementovat v konkrétním DBMS .
****
> **Zdrojové úryvky k odstavci výše**
>
...
Řeší se zde organizace datových souborů, indexy, výkonové aspekty, optimalizace přístupu a další technické detaily. 
...
****
Mezi jednotlivými částmi návrhu existuje zpětná vazba. Pokud se při fyzickém návrhu ukáže, že některé části modelu vedou k problémům, je nutné vrátit se zpět k návrhu logickému. 
Podobně může změna požadavků uživatelů vyvolat úpravu konceptuálního modelu a následně i dalších úrovní. 
...
****
Z pohledu této práce je právě členění návrhu na konceptuální, logickou a fyzickou úroveň, vhodné také jako východisko pro pozdější hodnoticí kritéria. 
Nástroj určený pro návrh databázového systému by měl umožnit srozumitelně zpracovávat požadavky, převést je do konzistentního schématu a podle potřeby podpořit i technickou implementaci v konkrétním databázovém prostředí.
Právě míra podpory těchto krocíchpředstavuje jeden z praktických rozdílů mezi porovnávanými nástroji (Carvalho et al., 2022; Rosenthal a Reiner, 1994).
****
> **Zdrojové úryvky k odstavci výše**
>
